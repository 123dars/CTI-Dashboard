...
def get_sample_threats():
    ...
    return [
        ...
        {"ioc": f"{random.randint(100000, 999999)}a{random.randint(100000, 999999)}b{random.randint(100000, 999999)}c", "type": "md5_hash",
         "threat": "ransomware", "malware": ["LockBit"],
         "confidence": random.randint(95, 100), "source": "HybridAnalysis",
         "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
        {"ioc": f"{rand_ip2}:{random.randint(1024, 65535)}", "type": "ip:port", "threat": "ransomware",
         ...
         "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
        {"ioc": f"http://spam-site-{rand_id}.xyz/ad", "type": "url",
         "threat": "spam", "malware": ["adware"],
         "confidence": random.randint(10, 40), "source": "URLhaus",
         "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S")},
